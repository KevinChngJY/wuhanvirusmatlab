function [table_overall_a,table_overall_b_Case, table_overall_b_Death,table_singapore] = covid19data()
    warning("off")
    ...
    c_singaporeinfo=webread("https://spreadsheets.google.com/feeds/list/1lwnfa-GlNRykWBL5y7tWpLxDoCfs8BvzWxFjeOZ1YJk/2/public/values?alt=json");
    % a_tableinfo
    ...
    try
        table_singapore = table(...
        [string(c_singaporeinfo.feed.entry.gsx_asof.x_t);string(c_singaporeinfo.feed.entry.gsx_newcases.x_t);string(c_singaporeinfo.feed.entry.gsx_casessofar.x_t);string(c_singaporeinfo.feed.entry.gsx_recovered.x_t);string(c_singaporeinfo.feed.entry.gsx_deceased.x_t);string(c_singaporeinfo.feed.entry.gsx_stillinhospital.x_t)],....
        'VariableNames',{'Singapore'},...
        'RowNames',{'Date' 'New cases announced' 'Confirmed cases so far' 'Recovered' 'Still in hospital' 'Deaths'});
    catch
        table_singapore = [];
    end
end
